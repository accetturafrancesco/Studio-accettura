...
if st.session_state.view == "lista":
    ...
    if not pazienti:
        ...
        for p in pazienti:
            ...
            col_p, col_b, col_del = st.columns([5, 1, 1])
            with col_p:
                st.markdown(f"""
            ...
            with col_del:
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("🗑", key=f"del_p_{p['id']}", use_container_width=True,
                             help="Elimina paziente e tutte le sedute"):
                    st.session_state[f"confirm_del_{p['id']}"] = True
                    st.rerun()
            if st.session_state.get(f"confirm_del_{p['id']}"):
                st.warning(f"⚠️ Eliminare **{p['nome']} {p['cognome']}** e tutte le sue sedute? Operazione irreversibile.")
                c1, c2 = st.columns(2)
                with c1:
                    if st.button("Sì, elimina", key=f"yes_del_{p['id']}", type="primary", use_container_width=True):
                        db.delete_paziente(p["id"])
                        st.session_state.pop(f"confirm_del_{p['id']}", None)
                        st.rerun()
                with c2:
                    if st.button("Annulla", key=f"no_del_{p['id']}", use_container_width=True):
                        st.session_state.pop(f"confirm_del_{p['id']}", None)
                        st.rerun()
